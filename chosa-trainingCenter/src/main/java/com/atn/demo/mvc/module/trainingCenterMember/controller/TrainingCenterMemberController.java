...
public class TrainingCenterMemberController {
	...
	public ModelAndView listOfAllMembrePage(
		...
		if (userName != null) {
			List<TrainingCenterMember> listOfMembreBystatus = trainingMemberCenterService
					.gettrainingCenterMemberByUserName(userName);

			model.addObject("listOfMembreBystatus", listOfMembreBystatus);

		}else if(memberRole != null) {
		List<TrainingCenterMember> listOfMembreBystatus = trainingMemberCenterService
				.gettrainingCenterMemberByStatus(memberRole, trainingcenter);
		model.addObject("listOfMembreBystatus", listOfMembreBystatus);
		}
	...
	@RequestMapping(value = { "/trainingCenter/membersearch" }, method = RequestMethod.GET)
	public ModelAndView searchProgram(
		...
		model.setViewName("redirect:/trainingCenter/member?userName=" + userName);
		return model;
	}
	...
	public ModelAndView listOfTeacherPage(
		...
		TrainingCenter trainingcenter = trainingCenterService
				.getconnectedTrainingCenter();
		TrainingCenterClass tc_class = trainingCenterClasseService
	...
	public ModelAndView listOfClassesByTeacherPage(
		...
		List<TrainingCenterClass> listOfClassesByTeacher = trainingCenterClasseService
				.getClassByTeacher(userTeacher.getId());
	...
	public ModelAndView getProgramByClassPage(
		...
		TrainingCenterProgram program = trainingcenterProgramService
				.findById(programId);
	...
	public ModelAndView subscribeTeacherToClassePage(
			...
			@RequestParam("teacherID") Long teacherID) {
		ModelAndView model = new ModelAndView();
		trainingMemberCenterService.suscribeTeacherToClass(teacherID,
				trainingcenterClassId);
	...
	public ModelAndView ValidorBloquedAccountMemberPage(
		...
		ModelAndView model = new ModelAndView();
		trainingMemberCenterService.validOrBloquedTrainingCentrerMember(
				memberId, statusMember);
		model.setViewName("redirect:/trainingCenter/member?memberRole="
				+ statusMember);
		return model;
	}
	...
	public ModelAndView saveOrUpdateMemberPage(
		...
		User userSave = userService.inscription(user, file, "/CHOSA-TC");
		TrainingCenter trainingCenter = trainingCenterService
				.getconnectedTrainingCenter();
