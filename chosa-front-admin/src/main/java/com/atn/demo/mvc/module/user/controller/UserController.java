...
TemplateMailService emailService;
	...
	public ModelAndView welcomePage() {
		...
		{
			TrainingCenterMember member=tariningCenterMemberService.getMemberConnected();
if(member!=null){
			if(member.getRole().equals(RoleTrainingCenter.TEACHER)){
				model.setViewName("redirect:/teacher/classes");
			}else{
	...
	public ModelAndView saveOrUpdatePage(
		...
		userService.inscription(user, file,"/CHOSA-ADMIN");
		model.setViewName("redirect:/login");
		return model;
	}
	...
	public ModelAndView forgetPasswordActionPage(@ModelAttribute("user") User user, HttpServletRequest request) {
		ModelAndView model = new ModelAndView();
	...
	public ModelAndView validateAccount(@RequestParam(name = "mailUser") String mailUser,
			@RequestParam(name = "token") String token) {
		ModelAndView model = new ModelAndView();
		
		User user = userService.findByEmail(mailUser);
		boolean succes = (user != null) && user.getToken().equals(token);
		if(succes)
			userService.validateAccountUser(mailUser);
